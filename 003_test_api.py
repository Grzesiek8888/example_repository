...
def test_api_returns_not_found_for_bad_url1():
    """
...
def test_can_get_task():
    """GET https://todo.pixegami.io/get-task/task_fb0c7dc55175469bab1e2699dbbd6653"""
    response = requests.get(url=ENDPOINT + "/get-task/" +"task_fb0c7dc55175469bab1e2699dbbd6653")
...
def can_list_tasks():
    """GET https://todo.pixegami.io/list-tasks/Alamakota+tmp
    Napisz test do wylistowania tasks
    """
    random_user = "Alamakota_tmp" + str(random.randint(1000000, 9999999))
    task_body = {"content": "Nakarm kota",
                 "user_id": "random_user",
                 "task_id": "01",
                 "is_done": False}
...
def test_can_delete_task():
    ...
    """
    response = requests.get(url=ENDPOINT + "/get-task/" + "unique_task_id32432")
    ...
    response_create = requests.put(url=ENDPOINT + "/create-task", json=task_body)
    assert response_create.status_code == 200
    #2 get task id from create body
    body_create_response = response_create.json()
    unique_task_id = body_create_response["task"]["task_id"]
    print(unique_task_id)
